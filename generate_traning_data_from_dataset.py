...
dataset_folder = "C:\\Users\\calle\\projects\\ProPainter\\datasets\\youtube-vos\\mkvs"
dataset_org_imgs = "C:\\Users\\calle\\projects\\ProPainter\\datasets\\youtube-vos\\JPEGImages\\"
dataset_right_masks = "C:\\Users\\calle\\projects\\ProPainter\\datasets\\youtube-vos\\right_masks\\"
dataset_left_masks = "C:\\Users\\calle\\projects\\ProPainter\\datasets\\youtube-vos\\left_masks\\"

os.makedirs(dataset_right_masks, exist_ok=True)
os.makedirs(dataset_left_masks, exist_ok=True)
depth_folder = dataset_folder + "\\"
...
def make_sample(depth, img_rgb, cam_matrix, do_right, simulate_convergense, ipd_baseline, ground_truth_side, virtual_org_side, output_W, output_H, transform_to_ref):
    ...
    edge_mask_l, edge_mask_r = depth_map_tools.steep_disparity_lr(generated_right_depth, cam_matrix, parallax_shift=ipd_baseline)
    #full_edge_mask = depth_map_tools.steep_mask_disparity(generated_right_depth, cam_matrix, parallax_shift=ipd_baseline)
    ...
    edge_ids_r = first_morph_bg_ids[edge_mask_r]
    edge_ids_l = first_morph_bg_ids[edge_mask_l]
    ...
    r_edge_mask = np.isin(ids_duble_morp, edge_ids_r)
    l_edge_mask = np.isin(ids_duble_morp, edge_ids_l)
    ...
    H_, W_ = r_edge_mask.shape   # infer dimensions from mask
    img = np.zeros((H_, W_, 3), dtype=np.uint8)

    img[hole_mask2x, 0] = 255
    
    if not do_right:
        #edge_mask_r2x = np.repeat(np.repeat(edge_mask_r, 2, axis=0), 2, axis=1)
        hole_mask2x = hole_mask2x & r_edge_mask
        hole_mask2x = r_edge_mask #overide hole mask with new simpler hole mask
    else:
        #edge_mask_l2x = np.repeat(np.repeat(edge_mask_l, 2, axis=0), 2, axis=1)
        hole_mask2x = hole_mask2x & l_edge_mask
        hole_mask2x = l_edge_mask #overide hole mask with new simpler hole mask 
    
    
    # apply masks
    
    #img[hole_mask2x, 1] = 255
    ...
    #show_imgs([normals_out])
    
    return out_generated, normals_out, None
...
test_masks = os.listdir(mask_root)
...
def convert_to_training_data(img_path, from_zip=False, zip_ref=None):
    ...
    depth_base_name = base_name + '_depth.mkv'
    
    xfov_base_name = depth_base_name + "_xfovs.json"
    ...
    #If a video is alredy in the test masks we dont need to create it again
    if name_only in test_masks:
        return
    
    org_imgs = [f for f in os.listdir(dataset_org_imgs + name_only) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    org_imgs.sort()
    
    
    dataset_right_masks_vid = dataset_right_masks + name_only + "\\"
    dataset_left_masks_vid = dataset_left_masks + name_only + "\\"
    
    os.makedirs(dataset_right_masks_vid, exist_ok=True)
    os.makedirs(dataset_left_masks_vid, exist_ok=True)
    ...
    left_mask_imgs = [f for f in os.listdir(dataset_left_masks_vid) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    right_mask_imgs = [f for f in os.listdir(dataset_right_masks_vid) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    
    #skip if it is already done
    if len(org_imgs) == len(left_mask_imgs) and len(org_imgs) == len(right_mask_imgs):
        return
    ...
    is_video = False
    if extention == '.mp4' or extention == '.mkv':
        is_video = True
    #Read Data
    if from_zip:
        if is_video:
    ...
    output_W, output_H = int(width), int(height)
    
    
    #Get the depth
    depth_video_name = depth_folder + depth_base_name
    
    xfovs_file_name = depth_folder + xfov_base_name
    
    with open(xfovs_file_name) as jf:
        xfovs = json.load(jf)
    
    depth_rgb_frames, fps = depth_frames_helper.load_video_frames_from_path(depth_video_name)
    #print(depth_rgb_frames)
    depth_frames = [depth_frames_helper.decode_rgb_depth_frame(f, 100.0, True) for f in depth_rgb_frames]
    
    #Depth is only saved in 16bit so we blur it a bit to counteract issues with quantization (Not IDEAL XXXXXX) To fix this we should 1. save depth in original resolution. 2. Increse bit rate
    depth_frames = [cv2.GaussianBlur(f, (5, 5), 1.0) for f in depth_frames]
    ...
    data = ['ipd_baseline:', ipd_baseline, 'simulate_convergense:', simulate_convergense, 'ground_truth_side:', ground_truth_side]
    ...
    for frame_no, _ in enumerate(depth_frames):
    
        org_name = org_imgs[frame_no] + ".png"
        
        left_name = dataset_left_masks_vid + org_name
        right_name = dataset_right_masks_vid + org_name
        if os.path.exists(left_name) and os.path.exists(right_name):
            continue
        ...
        cam_matrix = depth_map_tools.compute_camera_matrix(xfovs[frame_no], None, width, height)
        
        #generate mask as if the ground truth was the left camera
        out_generated, normals_out, encoded_depth_bgr24bit =  make_sample(depth_frames[frame_no], rgb_frames[frame_no], cam_matrix, True, simulate_convergense, ipd_baseline, 'not used', 'not used', output_W, output_H, cam_extrin)
        
        cv2.imwrite(left_name, cv2.cvtColor(normals_out, cv2.COLOR_RGB2BGR))
        
        
        #generate mask as if the ground truth was the right camera
        out_generated, normals_out, encoded_depth_bgr24bit =  make_sample(depth_frames[frame_no], rgb_frames[frame_no], cam_matrix, False, simulate_convergense, ipd_baseline, 'not used', 'not used', output_W, output_H, cam_extrin)
        
        cv2.imwrite(right_name, cv2.cvtColor(normals_out, cv2.COLOR_RGB2BGR))
        ...
        #cv2.imwrite(img_output_normal_gen, cv2.cvtColor(normals_out, cv2.COLOR_RGB2BGR))
...
def process_dataset_folder(dataset_path):
    ...
    exts = (".jpg", ".jpeg", ".png", ".bmp", ".tiff", ".webp", '.mp4', ".mkv")
    ...
    if os.path.isdir(dataset_path):
        ...
        for root, _, files in os.walk(dataset_path):
            for fname in files:
                if fname.lower().endswith(exts) and not fname.lower().endswith('_depth.mkv'):
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(convert_to_training_data, img_path): idx
            ...
            for i, future in enumerate(as_completed(futures)):
                ...
                try:
                    ...
                    print(f"[{i+1}/{len(image_paths)}] Done: {result}")
                except Exception as e:
                    print(f"[ERROR] {image_paths[idx]} failed: {e}")
